...
export function exerciseMuscleMap(exercise) {
  ...
  if (exercise.bodyMapUrl) return `<section class="muscle-map muscle-map--image muscle-map--${group}" aria-label="Músculos trabajados: ${exercise.primaryMuscles.join(', ')}"><div class="muscle-map__figure muscle-map__figure--image"><img src="${exercise.bodyMapUrl}" alt="Mapa corporal de zonas trabajadas en ${exercise.name}" loading="lazy" /></div><div><span class="eyebrow">ZONAS TRABAJADAS</span><b>${exercise.primaryMuscles.join(' · ')}</b><p>Principales en lima · secundarios en verde suave</p></div></section>`;
